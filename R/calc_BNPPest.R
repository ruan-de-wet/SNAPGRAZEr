...
#' @param ANPPest (equivalent to Pg) Total seasonal grass production for the given grazing management system. Output of calc_ANPPest() or calc_Pg().
...
calc_BNPPest = function(RAIN, MAT, ANPPest, Sk, S0 = 0.1*Sk, APCcorrection = FALSE, DEPTH = 30) {

  APC = ifelse(APCcorrection, 0.291, 1)
  BNPPest = (0.602*RAIN-0.00038*RAIN^2+5.88*MAT)*(ANPPest/(Sk-S0))*APC*(DEPTH/40)
  return(BNPPest)
